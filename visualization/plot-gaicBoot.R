sGs <- readRDS("Data/ThesisData/sTs_Bs.rds")
nGs <- readRDS("Data/ThesisData/nTs_Bs.rds")
tGs <- readRDS("Data/ThesisData/tTs_Bs.rds")
tGs_Comp <- readRDS("Data/ThesisData/tTs_Comp_Bs.rds")
tGs_Diag <- readRDS("Data/ThesisData/tTs_Diag_Bs.rds")
...
pdf(file="~/gaicPlots-Boot.pdf", width=14, height=7)
...
abline(v=cutoffs[which(sD==min(sD[1:30]))], col="black", lty=2, lend=3)
text(x=cutoffs[which(sD==min(sD[1:30]))+2], y=round(min(sD)), labels = cutoffs[which(sD==min(sD[1:30]))])

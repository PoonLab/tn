...
tGs_Comp <- readRDS("Data/ThesisData/tGs_Comp.rds")
...
#Number of Clusters
getGAIC <- function(iGs) {sapply(iGs, function(x){x$a$gaic})}

cutoffs <- seq(0,0.04,0.0008)

pdf(file="~/gaicPlots-graphs.pdf", width=14, height=7)
par(mfrow=c(1,2),mar=c(5,6,4,1)+.1)

#PlotA
plot(x=0, y=0, type='n', xlab="TN93 threshold", ylab="AIC Difference (Proposed - Null)", xlim=c(0,0.04), ylim=c(-85, 30),
...
abline(h=seq(20, -80, -20), col='white', lwd=3, lend=2)
abline(h=seq(10, -90, -20), col='white', lend=3)
